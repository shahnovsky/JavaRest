...
public class DeliveryArea implements Serializable {
	...
	public Set<Delivery> getRegularDeliversNotSent() {
		HashSet<Delivery> deliversNotSent = new HashSet<Delivery>();
		for (Delivery d : delivers)
			if ((d instanceof RegularDelivery) && (d.isDelivered() == false))
				deliversNotSent.add(d);
		return Collections.unmodifiableSet(deliversNotSent);
	}
