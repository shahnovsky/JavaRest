...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		try {
			File serialFile = new File("RestDatabase.ser");
			if (serialFile.exists()) {
				FileInputStream fileIn = new FileInputStream("RestDatabase.ser");
				ObjectInputStream in = new ObjectInputStream(fileIn);
				Restaurant.setInstance((Restaurant) in.readObject());
				in.close();
				fileIn.close();
			} else
				System.out.println("Could not find the database file. Starting from scratch");
