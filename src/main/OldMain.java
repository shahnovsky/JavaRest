...
public class OldMain {
	...
	static {
		...
		commands.put("addCustomer", (section, args) -> {
			...
			Customer c = new Customer(args[0], args[1], date, Gender.valueOf(args[5]), Neighberhood.valueOf(args[6]),
		...
		commands.put("addCook", (section, args) -> {
			LocalDate date = LocalDate.of(Integer.parseInt(args[2]), Integer.parseInt(args[3]),
					Integer.parseInt(args[4]));
			Cook c = new Cook(args[0], args[1], date, Gender.valueOf(args[5]), Expertise.valueOf(args[6]),
		...
		commands.put("addDeliveryPerson", (section, args) -> {
			...
			DeliveryPerson dp = new DeliveryPerson(args[0], args[1], date, Gender.valueOf(args[5]),
					Vehicle.valueOf(args[6]), da);
		...
		commands.put("addComponent", (section, args) -> {
			Component c = new Component(args[0], Boolean.parseBoolean(args[1]), Boolean.parseBoolean(args[2]),
		...
		commands.put("addDelivery", (section, args) -> {
			...
			if (o != null)
				orders.add(o);
			LocalDate deliveredDate = LocalDate.of(Integer.parseInt(args[6]), Integer.parseInt(args[7]),
		...
		commands.put("addExpressDelivery", (section, args) -> {
			...
			LocalDate deliveredDate = LocalDate.of(Integer.parseInt(args[5]), Integer.parseInt(args[6]),
		...
		commands.put("getDeliveriesByPerson", (section, args) -> {
			...
			if (result.isEmpty()) {
				...
				MyFileLogWriter
						.println("The deliveries for delivery person " + dp.getId() + " in month " + month + " is:");
		...
		commands.put("getNumberOfDeliveriesPerType", (section, args) -> {
			...
			if (result.isEmpty()) {
				...
				MyFileLogWriter.println("Has " + result.get("Regular delivery") + " regular deliveries and "
						+ result.get("Express delivery") + " express deliveries\n");
			}
		});
	...
	public static void main(String[] args) throws IOException {
		...
		File myObj = new File("C:\\Users\\Orit\\eclipse-workspace\\r\\RestDatabase.ser");
		if (myObj.delete()) {
